...
# You are tasked with implementing a method partition_list(self, x) that will take an integer x and partition the linked list such that 
...
# These two linked lists will be made up of the original nodes from the linked list that is being partitioned, one for nodes less than x and one for nodes greater than or equal to x.  
...
class LinkedList:
    ...
    # course solution
    # The partition_list method takes in a value x and partitions the linked list such that 
    # all nodes with values less than x come before nodes with values greater than or equal to x.
    # The method creates two dummy nodes, dummy1 and dummy2, which will be used to hold the two partitions. 
    # The prev1 and prev2 variables are used to keep track of the last node of each partition, initially pointing to their respective dummy nodes. 
    # The current variable starts at the head of the original list.

    # The method then iterates through the original list using a while loop. 
    # For each node in the list, the method checks if its value is less than x. 
    # If it is, the node is added to the first partition by setting prev1.next to the current node and updating prev1 to point to the current node.
    ...
    # Once all nodes have been processed, the method sets prev2.next to None to ensure that the second partition ends with a null pointer. 
    # It then sets prev1.next to dummy2.next, which connects the end of the first partition to the beginning of the second partition.
